...
def initialize_network(n_inputs, n_hidden1,n_hidden2, n_outputs):
    network = list()
    hidden1_layer = [{'weights':[random() for i in range(n_inputs + 1)]} for i in
        range(n_hidden1)]
    network.append(hidden1_layer)
    hidden2_layer = [{'weights': [random() for i in range(n_hidden1 + 1)]} for i in
                     range(n_hidden2)]
    network.append(hidden2_layer)

    output_layer = [{'weights':[random() for i in range(n_hidden2 + 1)]} for i in
        range(n_outputs)]
...
seed(1)#test algoritm
#main algorithm based on neutral network and deep leerning
...
network = initialize_network(n_inputs, 7,5, n_outputs)
